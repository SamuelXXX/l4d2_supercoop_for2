...
public void Event_PlayerSpawn(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if (GetClientTeam(client) == TEAM_SURVIVOR && !IsFakeClient(client) && Cvar_TimeBot.FloatValue >= 0)
	{
		...
		if (!SteamIDs.GetValue(SteamID, Allowed) || Allowed == 1)  // If can't find the entry in map or is allow but not yet timed
		{
			...
			if (Cvar_TimeBot.FloatValue > 2 && Cvar_PickBot.BoolValue) PrintToChat(client, "\x03[电脑接管]\x01 您还剩%d秒来选择接管一个电脑玩家！", Cvar_TimeBot.IntValue);
		}
	}
}
...
public Action Timer_RecordSteamID(Handle hTimer, Handle datapack)
{
	...
	if (client && Cvar_TimeBot.FloatValue > 2 && Cvar_PickBot.BoolValue) PrintToChat(client, "\x03[电脑接管]\x01 选择接管电脑时限已到，您已无法选择电脑进行接管.", Cvar_TimeBot.FloatValue);
}

bool VerifyCommand(int client)
{
	//Verifiy is command is appropriate
	if (client == 0)  {  ReplyToCommand(client, "\x03[电脑接管]\x01 只能在游戏中使用该命令.");  return false;} 
	if (!IsClientInGame(client) || IsFakeClient(client)) return false;
	if (GetClientTeam(client) != TEAM_SURVIVOR)  {  ReplyToCommand(client, "\x03[电脑接管]\x01 只有幸存者才能使用该命令.");  return false;} 	
	if (!Cvar_DeadBot.BoolValue || IsPlayerAlive(client) ) // Skip this if dead & l4d_sbs_dead is 1
	{
		if (!Cvar_PickBot.BoolValue) {ReplyToCommand(client, "\x03[电脑接管]\x01 接管命令未启用.");  return false;  }
		if (Cvar_TimeBot.FloatValue >= 0 && !PickAllowed(client)) {  ReplyToCommand(client, "\x03[电脑接管]\x01 在该轮战役中您无法再使用该命令.");  return false;}	
	}
	
	int AvailableBots = CountAvailableSurvivorBots();
	if (AvailableBots == 0)
	{
		ReplyToCommand(client, "\x03[电脑接管]\x01 找不到幸存的电脑玩家."); return false;
	}
	return true;
}
...
public Action Event_PlayerDeath(Handle event, const char[] name, bool dontBroadcast)
{
	...
	PrintToChat(client, "\x03[电脑接管]\x01 您已死亡，现在您可以使用!menu接管电脑！");
}
...
public int MenuHandler1(Menu menu, MenuAction action, int param1, int param2)
{
	...
	{ 
		case MenuAction_Select:  
		{
			...
			if (!VerifyCommand(client)) return;
			if (!PickedBot || !IsSurvivorBotValid(PickedBot) || GetIdlePlayer(PickedBot)) { PrintToChat(client, "\x03[电脑接管]\x01 该幸存者电脑已经无法接管.");  return;}
